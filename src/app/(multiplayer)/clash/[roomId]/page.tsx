...
export default function Page() {
  ...
  useClashLogsManager()
  const { allParticipantsSubmitted, closeRound, openNextRound, cloneRoom } = useRoomUtils()
  ...
  useEffect(() => {
    if (!session?.user) {
      alertDialog({
        ...
        subtitle: 'You must be logged in to join a Clash room.'
      })
  ...
  if (room && isPrivate && !authorized && session?.user?.id && room.createdBy !== session.user.id) {
    return <RoomAuthGate roomId={roomId as string} room={room} onCancel={() => router.push('/clash')} />
  }

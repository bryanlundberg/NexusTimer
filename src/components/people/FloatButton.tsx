          ...
          <TableBody>
            {users.map((user) => (
              <TableRow key={user._id}>
                <TableCell className="font-medium"><Button
                  variant={'ghost'}
                  size={'icon'}
                  onClick={() => removeUser(user._id)}
                ><TrashIcon/></Button></TableCell>
                <TableCell>
                  <Avatar>
                ...
                <TableCell>{user?.backup?.updatedAt ? formatDistance(new Date(user?.backup?.updatedAt), new Date(), { addSuffix: true }) : 'Never'}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>

...
import { Globe2, Lock, Play, ShieldCheck, TimerReset, Users, Clock, ListOrdered } from 'lucide-react';
...
export default function RoomListItem({ room, onJoin }: { room: Room, onJoin: (room: Room) => void }) {
  const { mmss, remainingMs } = useCountdown(room?.preparationFinalizationTime);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 px-6">
        ...
        <div className="flex items-center gap-2 justify-end w-full">
          <div className="flex items-center flex-wrap gap-2 text-xs text-muted-foreground justify-end">
            ...
            <span className="inline-flex items-center gap-1"><ListOrdered className="size-4"/> Rounds {room.totalRounds}</span>
            <span className="inline-flex items-center gap-1"><Clock className="size-4"/> Closes in {remainingMs !== undefined ? mmss : '--:--'}</span>
          </div>
          {room.status === RoomStatusEnum.IDLE && (
            <Button onClick={() => onJoin(room)} size="sm" className="inline-flex items-center gap-1">
